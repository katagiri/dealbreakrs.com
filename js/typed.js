...
!function($){
	...
	var Typed = function(el, options){
		...
		// number in which to stop going through array
		// set to strings[] array (length - 1) to stop deleting after last string is typed
		self.stopArray = self.strings.length; // ZK took off a "-1" from the right hand side of the equasion
		...
		Typed.prototype =  {
			...
			, typewrite: function(curString, curStrPos){
				...
				else if (self.arrayPos == -1) { // if it would have been the last then make the back delay longer
					self.backDelay = 7500;
				}
				else{ self.backDelay = 500; }
				...
				setTimeout(function() {
					...
					if (self.arrayPos < self.strings.length){
						...
						// else, keep typing
							...
							if (self.arrayPos == self.strings.length-1) { // if it would have been the last, then reset to -1 so that when it adds one it will start back at 0
								self.arrayPos = -1;
							}
